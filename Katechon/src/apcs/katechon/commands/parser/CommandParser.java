...
public class CommandParser
{
	...
	/**
	 * Parses a command into various valid possibilities as specified by the command
	...
	/**
	 * Populates a set of lists of strings full of the potential command possibilities of a given set of command arguments. For example, if you cmd arguments is ["a", "{b|c}", "d"] You will get "a b d" and "a c d".
	 * @param versionList Each version of a command is represented as a list of strings. This is a set of all of the versions that it found.
	 * @param workingVersion The method needs to be supplied an empty arraylist to start building and expanding off of as it finds new versions to add to the versionList
	 * @param index The index to start searching at. You will probably want to set this to 0.
	 * @param cmdStringParts The array of strings that is being searched for the various valid command possibilities (versions)
	 */
	...
	/**
	 * Parses a variant into an array of possible options. For instance, {a|b|cd ef g} would be parsed to an array of [a, b, cd ef g].
	...
	private static String[] parseArgumentOptions(String variant)
	{
		...
		//This is one of the options that is added to the options list when it is completed.
	...
	/**
	 * This will split a command string int o an array, but will maintain the integrity of variants (they look like {a|b|c})
	 * @param cmdString
	 * @return
	 */
	private static String[] safeSplit(String cmdString)
	{
		...
		for(int ii = 0; ii < chars.length; ii++)
		{
			...
			case ' ':
				...
				//we are going though a variant so we're going to fall though and append it to the part builder
			default:
				partBuilder.append(c);
			}
		}
